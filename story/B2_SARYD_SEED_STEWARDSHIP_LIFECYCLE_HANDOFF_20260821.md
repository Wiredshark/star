...
A3 should integrate only after the branch passes repository-native story/simulation/style and production build/save-load gates. B3 should preserve the lifecycle invariant that dialogue-only B2 missions which merely persist state terminate with `decline`; `accept` is reserved for mission lifecycles that actually create gameplay objectives.
