...
DATA = ROOT / "data" / "wanderer" / "b2 wanderer return home choice compact.txt"
text = DATA.read_text(encoding="utf-8")
PREFIX = "B2 Wanderer Return Home Choice Compact:"
...
require(missions == [PREFIX + " Offer", PREFIX + " Review", PREFIX + " Child Remembers"],
...
require('has "B2 Wanderer Evacuation Recovery Compact: aftermath seen"' in text,
        "missing integrated evacuation-recovery aftermath dependency")
require('government "Wanderer"' in text, "missing Wanderer source scope")

OFFER = block('mission "' + PREFIX + ' Offer"', 'mission "' + PREFIX + ' Review"')
REVIEW = block('mission "' + PREFIX + ' Review"', 'mission "' + PREFIX + ' Child Remembers"')
AFTERMATH = block('mission "' + PREFIX + ' Child Remembers"')

routes = {
    "consent": PREFIX + " route present consent",
    "option": PREFIX + " route right without duty",
    "paired": PREFIX + " route paired records",
}
for label, condition in routes.items():
    section = block('\t\t\tlabel ' + label, '\t\t\tlabel ' + ({"consent":"option","option":"paired","paired":"decline"}[label]))
...
settlements = {
    "packet": PREFIX + " settlement portable home choice",
    "renewal": PREFIX + " settlement fresh choice renewal",
}
for label, condition in settlements.items():
    next_label = "renewal" if label == "packet" else None
    section = block('\t\t\tlabel ' + label, '\t\t\tlabel ' + next_label if next_label else 'mission "' + PREFIX + ' Child Remembers"')
...
for phrase in (
    "old address can remain true history without becoming present authority",
    "keeping an option alive is different from assigning a duty",
    "current residence and contact preference",
    "old addresses and evacuation intentions remain attributable history",
):
    require(phrase in text, f"missing continuity boundary: {phrase}")
require("general Wanderer rule" in text, "missing local-not-universal authority boundary")

print("B2 Wanderer Return Home Choice Compact validation: PASS")
