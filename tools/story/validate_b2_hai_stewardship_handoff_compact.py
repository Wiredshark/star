...
DATA = ROOT / "data/hai/b2 hai stewardship handoff compact.txt"
text = DATA.read_text(encoding="utf-8")

PREFIX = "B2 Hai Stewardship Handoff Compact:"
...
require(text.count('mission "B2 Hai Stewardship Handoff Compact:') == 3, "expected exactly three compact missions")
require('event "B2 Hai Stewardship Handoff Compact: Review Ready"' in text, "missing delayed Review event")
require('has "First Contact: Hai: offered"' in text, "missing Hai first-contact dependency")
require('has "Hai Stewardship Archive: offered"' in text, "missing B1 stewardship-history dependency")
require('government "Hai"' in text, "missing Hai source scope")
require("Tavi Heren" in text and "Jalen Orr" in text, "missing recurring mentor/successor characters")

# Persistence ownership: every direct assignment stays inside this B2 namespace.
...
OFFER = "B2 Hai Stewardship Handoff Compact: Offer"
REVIEW = "B2 Hai Stewardship Handoff Compact: Review"
AFTER = "B2 Hai Stewardship Handoff Compact: Jalen Remembers"
offer = block(OFFER, REVIEW)
review = block(REVIEW, AFTER)
after = block(AFTER)

routes = {
    "current": "route current inspection",
    "adoption": "route explicit adoption",
    "paired": "route paired records",
}
labels = ["current", "adoption", "paired", "decline"]
for index, (label, state) in enumerate(routes.items()):
    ...
    require(section.count(f'"{PREFIX} {state}" = 1') == 1, f"{label} must write its route exactly once")
...
# Review lifecycle and route wiring. Current-inspection is the deliberate default path.
...
require(f'has "{PREFIX} route explicit adoption"' in review, "Review must branch on adoption route")
require(f'has "{PREFIX} route paired records"' in review, "Review must branch on paired route")
require(f'has "{PREFIX} route current inspection"' not in review, "current-inspection route should remain deliberate default Review path")

settlements = {
    "packet": "settlement portable stewardship packet",
    "versions": "settlement versioned custody",
}
review_labels = ["packet", "versions"]
...
# Continuity/canon boundary: historical experience is preserved without becoming standing authority.
for fragment in (
    "mentorship survives a handoff without becoming permanent command",
    "present maintenance decision",
    "current instruction only when the present steward deliberately adopts it",
    "old handoff notes remain immutable history",
    "not universal Hai law",
):
    require(fragment in text, f"missing continuity invariant fragment: {fragment}")

print("PASS: B2 Hai Stewardship Handoff Compact")
