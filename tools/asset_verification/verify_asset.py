...
def candidate_attestation(repo: Path, candidate: Path, head: str) -> dict[str, Any]:
    ...
    if working_blob != blob:
        raise VerificationError(f"candidate bytes do not match HEAD {head}: expected blob {blob}, working blob {working_blob}")
    return {"path": rel, "git_blob": blob, "sha256": sha256_file(candidate), "size": candidate.stat().st_size}
...
def png_info(path: Path, session_start_ns: int) -> dict[str, Any]:
    ...
    return {"path": path.name, "sha256": sha256_file(path), "size": stat.st_size, "width": width, "height": height, "mtime_ns": stat.st_mtime_ns}


def run_command(label: str, command: list[str], repo: Path, env: dict[str, str], timeout: int) -> dict[str, Any]:
    started = time.time_ns()
    try:
        result = subprocess.run(command, cwd=repo, env=env, text=True, capture_output=True, timeout=timeout)
    ...
    if result.returncode:
        raise VerificationError(f"{label} command failed with exit {result.returncode}:\n{result.stderr[-2000:] or result.stdout[-2000:]}")
    return record
...
def cmd_capture(args: argparse.Namespace) -> int:
    ...
    if args.prepare_command:
        receipt["prepare"] = run_command("prepare", parse_command(args.prepare_command), repo, env, args.timeout)
        require_clean(repo)
    if args.binary:
        ...
        receipt["binary"] = {"path": str(binary), "sha256": sha256_file(binary), "size": binary.stat().st_size, "mtime_ns": binary.stat().st_mtime_ns}
    receipt["capture"] = run_command("capture", parse_command(args.capture_command), repo, env, args.timeout)
    require_clean(repo)
    ...
    if not screenshots:
        raise VerificationError("capture command succeeded but produced no fresh PNG in ASSET_VERIFY_CAPTURE_DIR")
    receipt["screenshots"] = screenshots
    receipt["session_end_ns"] = time.time_ns()
    receipt["worktree_clean_after"] = True
    receipt["status"] = "PASS"
    receipt["acceptance"] = {"status": "PROOF_VERIFIED", "reasons": [
    ...
    atomic_json(run_dir / "receipt.json", receipt)
    (evidence_root / "LATEST").write_text(run_id + "\n", encoding="utf-8")
    print(f"PASS {args.asset_id} {head} {run_dir / 'receipt.json'}")
    return 0


def verify_screenshot(run_dir: Path, record: dict[str, Any]) -> None:
    recorded_name = record["path"]
    if Path(recorded_name).name != recorded_name:
        raise VerificationError(f"invalid screenshot path in receipt: {recorded_name}")
    path = run_dir / "captures" / recorded_name
    if not path.is_file():
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    if args.require_current_head:
        ...
        if current_head != proof_head:
            raise VerificationError(f"proof is stale for current HEAD: receipt={proof_head}, current={current_head}")
    ...
    committed_bytes = subprocess.run(["git", "show", f"{proof_head}:{candidate['path']}"], cwd=repo, capture_output=True)
